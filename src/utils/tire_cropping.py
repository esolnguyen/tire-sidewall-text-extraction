...
from ultralytics import YOLO  # if you're using ultralytics YOLO
import cv2
import numpy as np
import os
...
def detect_tire_and_rim(yolo_model, image):
    ...
    if results and results[0].boxes:
        ...
        wheel_contour = boxes[0]
        rim_contour = boxes[1] if len(boxes) > 1 else None
...
def detect_tire_and_rim(
    ...
    """
    Detect wheel and rim, create a segmentation-style visualization
    highlighting the detected parts, and optionally save it.

    Returns:
        (wheel_contour, rim_contour, vis_image) on success
        None on failure
    """
    ...
    if len(masks_xy) < 2:
        print(
            f"Not enough masks detected. Found {len(masks_xy)}, "
            "need at least 2 for wheel and rim."
        )
        return None

    # Sort by area (largest = wheel, second = rim)
    sorted_masks = sorted(
        masks_xy,
    ...
    # --- Build binary masks for wheel and rim ---
    h, w = image.shape[:2]
    ...
    # --- Create colored overlay on the detected parts ---
    overlay = image.copy()

    # Colors are in BGR (OpenCV default)
    wheel_color = (0, 255, 0)  # green
    rim_color = (0, 0, 255)  # red

    overlay[wheel_mask == 255] = wheel_color
    overlay[rim_mask == 255] = rim_color

    # Blend overlay with original image for transparency
    alpha = 0.6
    seg_overlay = cv2.addWeighted(overlay, alpha, image, 1 - alpha, 0)

    # --- Optional: darken background so detected parts are highlighted ---
